...
int exitWait(void) {
  ...
  printf(1, "\n  Parts a & b & c) testing getppid(), exit(int status), and wait(int* status):\n");
  ...
  for (i = 0; i < 2; i++) {
    ...
    if (pid == 0) { // only the child executed this code
      if (i == 0)
      {
        printf(1, "\nThis is child with PID# %d with Parent PID# %d and I will exit with status %d\n",getpid(), getppid(), 0);
        exit(0);
      }
      else
      {
	      printf(1, "\nThis is child with PID# %d and Parent PID# %d and I will exit with status %d\n",getpid(), getppid(), -1);
        exit(-1);
      } 
...
void printInfo(void){
    printf(1,"\n Parts d) Testing print function:\n");
    int i;
    for(i = 0; i < 5; i++){
      fork();
    }
